...
static int rtk_pcie2_hw_initial(struct device *dev)
{
	...
	if (gpio_is_valid(pcie_gpio_iso)) {
		ret = gpio_direction_output(pcie_gpio_iso, 1);
		mdelay(100);
	}
	/* after phy mdio set */
...
static int rtk_pcie2_probe(struct platform_device *pdev)
{
	...
	pcie_gpio_iso = of_get_gpio_flags(pdev->dev.of_node, 1, NULL);
	if (gpio_is_valid(pcie_gpio_iso)) {
		ret = gpio_request(pcie_gpio_iso, "pcie_gpio_iso");
		if (ret < 0)
			dev_err(&pdev->dev, "%s: can't request gpio %d\n",
				__func__, pcie_gpio_iso);
	} else
		dev_err(&pdev->dev, "%s: gpio %d is not valid\n",
			__func__, pcie_gpio_iso);
...
static int rtk_pcie2_suspend(struct device *dev)
{
	...
	if (RTK_PM_STATE == PM_SUSPEND_STANDBY) {
		...
		mdelay(100);
		if (gpio_is_valid(pcie_gpio_iso)) {
			gpio_direction_output(pcie_gpio_iso, 0);
			mdelay(100);
		}
		reset_control_assert(rstn_pcie1_stitch);
		reset_control_assert(rstn_pcie1);
...
static void rtk_pcie2_shutdown(struct platform_device *pdev)
{
