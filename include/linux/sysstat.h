...
#define ASUSTOR_SYS_RECORD_TIMES	64
#define ASUSTOR_CPU_COUNT_MAX		16
#define ASUSTOR_NETIF_COUNT_MAX		16
...
typedef struct _T_ASUSTOR_NETIF_LOAD_ {
	...
	uint64_t 	kbTxTotal;
	uint64_t 	kbRxTotal;
	uint64_t 	rykbTx[ASUSTOR_SYS_RECORD_TIMES];	
	uint64_t 	rykbRx[ASUSTOR_SYS_RECORD_TIMES];
...
typedef struct _T_ASUSTOR_MDEV_LOAD_ {
	...
	uint64_t 	rykbRead[ASUSTOR_SYS_RECORD_TIMES];	
	uint64_t 	rykbWrite[ASUSTOR_SYS_RECORD_TIMES];
...
typedef struct _T_ASUSTOR_SYSTEM_STAT_FOR_COUNT_{
	...
	uint8_t						iMdevCount;
	uint32_t					iMemTotal;
	T_ASUSTOR_CPU_USAGE_TOTAL 	rytCpuUsage[ASUSTOR_SYS_RECORD_TIMES];
	T_ASUSTOR_MEM_USAGE 		rytMemUsage[ASUSTOR_SYS_RECORD_TIMES];
...
extern int Add_Asustor_Diskdev_Stat(const char *pszDevName);

extern int Remove_Asustor_Diskdev_Stat(const char *pszDevName);
