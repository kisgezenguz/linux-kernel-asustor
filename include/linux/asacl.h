...
/** \brief	Reference API: posix_acl_dup() of inlcude/linux/posix_acl.h. */
...
/** \brief	Reference API: posix_acl_release() of inlcude/linux/posix_acl.h. */
...
/** \brief	To get the asacl of the specified inode.
 * 			Reference API: get_acl() of fs/posix_acl.c.
 */
 ...
 * 			Reference API: posix_acl_create() of fs/posix_acl.c.
 *
 * \param		ptParentInode	Pointer to the inode of the paraent dir of the new file.
 * \param		ptInode			The inode of the new file.
 * \param[out]	pptasAcl		Pointer to the buffer to get the returned ASACL obj.
 * \return		It returns 0 if successful, or returns a negative value if encountering an error.
 * 				-EINVAL			Invalid input parameter.
 * 				-ENOMEM			Out of memory.
 * 				-EALREADY		The new file is a symlink and don't have to set acl.
 */
extern int Create_Asacl_For_New_File(struct inode *ptParentInode, struct inode *ptInode, PPT_ASACL pptasAcl);

/** \brief	To estimate the ASACL xattr size of a new created file.
 *
 * \param	mMode			The mode of the new file.
 * \param	ptParentInode	The inode of the parent dir of the new file.
 * \return	It returns the estimated size in bytes of the ASACL xattr of the new file if successful,
 * 			or returns a negative value if encountering an error.
 */
...
/** \brief The ASACL chown handling function.
 *
 ...
 * 			Reference API: "posix_acl_chmod()" function of fs/posix_acl.c.
 *
...
/** \brief To get the corresponding xattr data size of a ASACL obj.
 *
 * \param	cnEntry	The number of the ACE entries of the specified ASACL obj.
 * \return	It returns the size in bytes of the corresponding xattr data.
 */
