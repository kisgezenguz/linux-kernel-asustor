...
#ifdef CONFIG_MMC_RTK_EMMC_PON
bool partition_pon_sem = false;

void rtkemmc_occupy_semaphore(void)
{
	...
	while(1) {
		if(readl(emmc_port->emmc_membase+EMMC_HD_SEM)==1) break;
			...
			printk(KERN_INFO "%s: Pon is occupying the semaphore,\
				pon opcode=%d, pon argument=0x%x, 0x98012030=0x%x, 0x98012032=0x%x, 0x9801203c=0x%x, 0x98012814=0x%x, 0x98012818=0x%x\n",
				__func__, CMD_IDX_MASK(readl(emmc_port->emmc_membase+EMMC_CMD_R)), readl(emmc_port->emmc_membase+EMMC_ARGUMENT_R),
				readw(emmc_port->emmc_membase+0x30), readw(emmc_port->emmc_membase+0x32),
				readw(emmc_port->emmc_membase+0x3c), readl(emmc_port->emmc_membase+0x814), readl(emmc_port->emmc_membase+0x818));
				usleep_range(10, 30);
		}
	}
	partition_pon_sem = true;
}
EXPORT_SYMBOL(rtkemmc_occupy_semaphore);

void rtkemmc_release_semaphore(void)
{
	struct mmc_host * mmc = mmc_host_local;
	struct rtkemmc_host *emmc_port = mmc_priv(mmc);

	rtkemmc_writel(0x0, emmc_port->emmc_membase+EMMC_HD_SEM);
	partition_pon_sem = false;
}
EXPORT_SYMBOL(rtkemmc_release_semaphore);
#endif
...
static void rtkemmc_cmdq_post_req(struct mmc_host *host, int tag, int err);
...
struct cmdq_node cmdq_data[32];

static void rtkemmc_cmdq_dump_dma_metadata(struct mmc_cmdq_req *cmdq_rq, u32 *desc, u8 read, int  dma_nents, int tag)
{
	cmdq_data[tag].read = read;
	cmdq_data[tag].blk_addr = cmdq_rq->blk_addr;
	cmdq_data[tag].blk_cnt = cmdq_rq->data.blocks;
	cmdq_data[tag].desc = desc;
	cmdq_data[tag].dma_nent = dma_nents;

	printk(KERN_INFO "%s: tag=%d, read=%d, blk_addr=0x%x, blk_cnt=%d, desc_addr=0x%x, dma_nent=%d\n",
		__func__, tag, cmdq_data[tag].read, cmdq_data[tag].blk_addr, cmdq_data[tag].blk_cnt, cmdq_data[tag].desc, cmdq_data[tag].dma_nent);
}

static void rtkemmc_cmdq_dma_debug(struct mmc_host *host, int tag)
{
	int idx=0;
	struct scatterlist *sg;
	int  dma_leng = 0;
	...
	struct cmdq_host *cq_host = (struct cmdq_host *)mmc_cmdq_private(host);
        struct mmc_request *mrq = cq_host->mrq_slot[tag];
        struct mmc_data *data = mrq->data;

	if(tag!=31) {
		printk(KERN_INFO "%s: tag=%d, read=%d, blk_cnt=%d, blk_addr=0x%x, desc_virt_addr=0x%x, dma_nent=%d\n",
			__func__, tag, cmdq_data[tag].read, cmdq_data[tag].blk_cnt, cmdq_data[tag].blk_addr, cmdq_data[tag].desc, cmdq_data[tag].dma_nent);

		for_each_sg(data->sg, sg, cmdq_data[tag].dma_nent, i) {
			dma_addr = sg_dma_address(sg);
			dma_leng = sg_dma_len(sg);
			base_addr = kmap_atomic(sg_page(sg));
			virt_addr = base_addr + sg->offset;

			printk(KERN_INFO "%s: nent%d, dma_addr=0x%x, dma_len=%d, virt_addr=0x%x, sg->offset=0x%x\n", __func__, i, dma_addr, dma_leng, virt_addr, sg->offset);

#if 0
			//dump scatterlist dma content
			for(idx=0;idx<dma_leng;idx+=4) {
				int idx2 = idx % 4096;  //page size is 4096
				if(idx2==0 && idx!=0) {
					kunmap_atomic(virt_addr-sg->offset);
					ret_page = phys_to_page(dma_addr+idx);
					virt_addr = kmap_atomic(ret_page) + sg->offset;
				}
				printk(KERN_INFO "%s phys_addr 0x%x, virt_addr 0x%x: 0x%x\n",__func__,  dma_addr+idx, virt_addr+idx2, rtkemmc_swap_endian(*(u32*)(virt_addr+idx2)));
			}
#else
			if(rtkemmc_swap_endian(*(u32*)(virt_addr+8))==0x12345678) {       //assume that input test pattern is 12345678
				for(idx=0; idx<dma_leng; idx+=4) {
					idx2 = idx % 4096;  //page size is 4096
					if( idx2==0 && idx!=0) {
						kunmap_atomic(base_addr);		//free the old page
						ret_page = phys_to_page((dma_addr+idx));
						base_addr = kmap_atomic(ret_page);
						virt_addr = base_addr + sg->offset;	//create another page
					}
					//dump the unexpected scatterlist dma data
					if(rtkemmc_swap_endian(*(u32*)(virt_addr+(idx2)))!=0x12345678) {
						printk(KERN_ERR "%s: DATA error detected !!!!!!!\n", __func__);
						printk(KERN_ERR "%s: tag=%d, read=%d, blk_cnt=%d, blk_addr=0x%x, desc_addr=0x%x, dma_nent=%d\n",
							__func__, tag, cmdq_data[tag].read, cmdq_data[tag].blk_cnt, cmdq_data[tag].blk_addr,
							cmdq_data[tag].desc, cmdq_data[tag].dma_nent);
						printk(KERN_ERR "%s: nent%d, dma_addr=0x%x, dma_len=%d, virt_addr=0x%x, sg->offset=0x%x\n",
							__func__, i, dma_addr, dma_leng, virt_addr, sg->offset);
						printk(KERN_ERR "%s virt_addr 0x%x: 0x%x\n", __func__, virt_addr+idx2-8 , rtkemmc_swap_endian(*(u32*)(virt_addr+idx2-8)));
						printk(KERN_ERR "%s virt_addr 0x%x: 0x%x\n", __func__, virt_addr+idx2-4 , rtkemmc_swap_endian(*(u32*)(virt_addr+idx2-4)));
						printk(KERN_ERR "%s virt_addr 0x%x: 0x%x\n", __func__, virt_addr+idx2   ,   rtkemmc_swap_endian(*(u32*)(virt_addr+idx2)));
						printk(KERN_ERR "%s virt_addr 0x%x: 0x%x\n", __func__, virt_addr+idx2+4 , rtkemmc_swap_endian(*(u32*)(virt_addr+idx2+4)));
						printk(KERN_ERR "%s virt_addr 0x%x: 0x%x\n", __func__, virt_addr+idx2+8 , rtkemmc_swap_endian(*(u32*)(virt_addr+idx2+8)));
						printk(KERN_ERR "%s virt_addr 0x%x: 0x%x\n", __func__, virt_addr+idx2+12,rtkemmc_swap_endian(*(u32*)(virt_addr+idx2+12)));
						rtkemmc_cmdq_halt(host, true);
						break;
					}
                                }
                        }
#endif
			kunmap_atomic(base_addr);
		}
	}
}
#endif
...
{
	struct cmdq_host *cq_host = (struct cmdq_host *)mmc_cmdq_private(host);
	struct mmc_request *mrq = cq_host->mrq_slot[tag+cq_host->start_slot];
	struct mmc_data *data = mrq->data;
...
#ifdef RTKMMEC_DMA_DEBUG
		rtkemmc_cmdq_dma_debug(host, tag+cq_host->start_slot);
#endif
		if (err)
			data->bytes_xfered = 0;
		else
...
#ifdef RTKMMEC_DMA_DEBUG
        rtkemmc_cmdq_dump_dma_metadata(cmdq_rq, desc, read, dma_nents, tag);
#endif
...
void pon_initial(struct rtkemmc_host *emmc_port, u32 start_addr, u32 end_addr, u32 id, u32 cmdq)
{
	...
	rtkemmc_writel((readl(emmc_port->mux_mis_membase + 0x4)& 0xfffc0001) | 0x0001B6DE, emmc_port->mux_mis_membase + 0x4); // pad mux, spi nand device
	...
	rtkemmc_write_swc(0x98012800, start_addr);
	rtkemmc_write_swc(0x98012804, end_addr);
	...
	if(readl(emmc_port->norst_membase)!=0x1234567) {
		gpio_direction_output(emmc_port->emmc_pon_toggle_gpio, 0);
		mdelay(1);
		rtkemmc_writel(0x1234567, emmc_port->norst_membase);
	}
	gpio_direction_output(emmc_port->emmc_pon_toggle_gpio, 1);
	gpio_direction_output(emmc_port->emmc_pon_gpio, 0);

	printk(KERN_ERR "%s: 0x98007104=0x%x, pon_start_addr=0x%x, pon_end_addr=0x%x",
		__func__, rtkemmc_read_swc(0x98007104), rtkemmc_read_swc(0x98012800), rtkemmc_read_swc(0x98012804));
}
#endif
...
void print_reg_info(struct rtkemmc_host *emmc_port)
{
	...
	printk(KERN_ERR "0x9801200e CMD_IDX = 0x%08x\n",readw(emmc_port->emmc_membase+EMMC_CMD_R));
...
{ 
	...
	printk(KERN_ERR "0x98012054 EMMC_ADMA_ERR_STAT_R = 0x%08x\n", readl(emmc_port->emmc_membase+EMMC_ADMA_ERR_STAT_R));
...
retry_L1:
	rtkemmc_clr_int_sta();	//clear error status, and we have saved the previous error status via rtkemmc_get_int_sta()
...
#if 0
	if(emmc_port->error_interrupt & 0xf) {
	}

	if(emmc_port->error_interrupt & 0x10) {
	}
#endif
	udelay(40);
	if((readl(emmc_port->emmc_membase + EMMC_PSTATE_REG) &0xf00000)!=0xf00000 || (readl(emmc_port->emmc_membase + EMMC_PSTATE_REG) & 0xf0)!=0xf0) {
		printk(KERN_INFO "%s: stop command check, EMMC_PSTATE_REG=0x%x\n", __func__, readl(emmc_port->emmc_membase + EMMC_PSTATE_REG));
		goto retry_L1;
	}
}
...
int rtkemmc_send_cmd6(struct rtkemmc_host *emmc_port, u32 args, u16 * state, int phase)
{
	...
	if(err ) {
#ifdef RTKEMMC_DEBUG
                printk(KERN_ERR "%s error: 0x98012030=0x%x, 0x98012032=0x%x, phase=%d\n",
...
void rtkemmc_phase_tuning(struct rtkemmc_host *emmc_port,u32 mode,int flag, int stage)
{
	u32 TX_window=0;
        u32 RX_window=0;
	u32 TX1_window=0;
	...
	if ((stage==TUNING_STAGE1 || stage==TUNING_STAGE_BOTH) && emmc_port->tx_tuning) {
	...
	if ((stage==TUNING_STAGE2 || stage==TUNING_STAGE_BOTH) && (mode == MODE_HS400||mode == MODE_DDR) && emmc_port->tx_tuning) {
...
#ifdef CONFIG_MMC_RTK_EMMC_PON
	if(get_rtd_chip_revision()!=RTD_CHIP_A00)	//workaround in hank A00, still has read/write timeout issue, only for pon testing
...
static int mmc_Tuning_DDR50(struct rtkemmc_host *emmc_port, u32 mode)
{
	...
	if(emmc_port->tx_tuning && emmc_port->rx_tuning) {	//device tree set kernel tuning for DDR50
		rtkemmc_phase_tuning(emmc_port,mode,1, TUNING_STAGE_BOTH);
		sync(emmc_port);
		mdelay(10);
	}
	else {
...
static void rtkemmc_dqs_tuning(struct mmc_host *host)
{
	...
	if(suspend == 1)
	{
		if(emmc_port->dqs_tuning == 1) {
			//in hank A00, dqs setting must be followed by cmd6
	...
	g_bTuning = 1;
		...
		if( rw_test_tuning(emmc_port, (emmc_port->emmc_tuning_addr/512))==0) {
			...
			pon_initial(emmc_port, emmc_port->pon_addr, emmc_port->pon_addr+((256*2*1024)/4*5),0x00232223, 1);
#else
			//currently, we disable command queue first, offset = 256x1024x1024/512/2048x512x5
			pon_initial(emmc_port, emmc_port->pon_addr, emmc_port->pon_addr+((256*2*1024)/4*5),0x00232223, 0);
#endif
#endif
			set_RTK_initial_flag(1);
			return;
...
retry:
	...
	for(i=0; i<0x20; i++) {
		...
		if(get_rtd_chip_revision()!=RTD_CHIP_A00) {
			...
			rtkemmc_phase_tuning(emmc_port, MODE_HS400, 0, TUNING_STAGE_BOTH);
		}
		else {
			rtkemmc_phase_tuning(emmc_port,MODE_HS400,0, TUNING_STAGE1);
			rtkemmc_dqs_delay_tap(emmc_port, (i<<1));
			rtkemmc_phase_tuning(emmc_port,MODE_HS400,0, TUNING_STAGE2);
		}
	...
	rtkemmc_send_cmd25(emmc_port, 512, dqs_tuning_blk_addr+1024, 1, hs400_data);
...
static void rtkemmc_set_ios(struct mmc_host *host, struct mmc_ios *ios)
{
	...
	if (ios->bus_width == MMC_BUS_WIDTH_8){
		...
		printk(KERN_INFO "%s: set bus width 8, host->mode=%d, 0x98012028=%08x\n", DRIVER_NAME, host->mode,readb(emmc_port->emmc_membase + EMMC_HOST_CTRL1_R));

#ifdef CONFIG_MMC_RTK_EMMC_PON  //we just write the fix value for fpga test, we will read the input from device tree after negotiate with android team later
		if(host->mode == MODE_SDR) {
#ifdef CONFIG_MMC_RTK_EMMC_CMDQ
			pon_initial(emmc_port, emmc_port->pon_addr, emmc_port->pon_addr+((256*2*1024)/4*5),0x00232223, 1);
#else
			//currently, we disable command queue first, offset = 256x1024x1024/512/2048x512x5
...
#ifdef RTKEMMC_PHASE_TRACE
			printk(KERN_ERR "trace_index=%d, trace_phase_bitmap=0x%x\n", trace_index, trace_phase_bitmap);
...
static void rtkemmc_timeout_timer(unsigned long data)
{
	...
	if(emmc_port->int_waiting)
	{
		...
		MMCPRINTF("========== C1 ==========\n");
		printk(KERN_ERR "%s: before clear signal interrupt\n", __func__);
		printk(KERN_ERR "%s: 0x98012038 NORMAL INTERRUPT SIGNAL EN= 0x%x\n", __func__, readw(emmc_port->emmc_membase+EMMC_NORMAL_INT_SIGNAL_EN_R));
		printk(KERN_ERR "%s: 0x9801203a ERROR INTERRUPT SIGNAL EN = 0x%x\n", __func__, readw(emmc_port->emmc_membase+EMMC_ERROR_INT_SIGNAL_EN_R));
		rtkemmc_hold_int_dec();
                rtkemmc_get_int_sta(&emmc_port->normal_interrupt, &emmc_port->error_interrupt);
		sync(emmc_port);
		printk(KERN_ERR "%s: normal_interrupt =%08x, error_interrupt=0x%08x\n",
				__func__, emmc_port->normal_interrupt, emmc_port->error_interrupt);
	}
	else {
		WARN_ON(1);
...
static void rtkemmc_dump_dma_metadata(struct sd_cmd_pkt *cmd_info, u32* des_base, int  dma_nents)
{
	q_data.idx = cmd_info->cmd->opcode;
	q_data.blk_addr = cmd_info->cmd->arg;
	q_data.blk_cnt = cmd_info->block_count;
	q_data.desc = des_base;
	q_data.dma_nent = dma_nents;

	printk(KERN_INFO "%s: idx=%d, blk_addr=0x%x, total_blk_cnt=%d, desc_virt_addr=0x%x, dma_nent=%d\n",
		__func__, q_data.idx, q_data.blk_addr, q_data.blk_cnt, q_data.desc, q_data.dma_nent);
}
static void rtkemmc_dma_debug(struct sd_cmd_pkt *cmd_info)
{
	if(cmd_info->cmd->opcode==17 || cmd_info->cmd->opcode==18 || cmd_info->cmd->opcode==24 || cmd_info->cmd->opcode==25) {
		int idx=0;
		struct scatterlist *sg;
		int  dma_leng = 0;
		volatile u32  dma_addr;
		...
		int i=0;
		struct page *ret_page;
		printk(KERN_INFO "%s: idx=%d, blk_addr=%d, blk_cnt=0x%x, desc_virt_addr=0x%x, dma_nent=%d\n",
				__func__, cmd_info->cmd->opcode, cmd_info->cmd->arg, cmd_info->block_count, q_data.desc, q_data.dma_nent);

		for_each_sg(cmd_info->data->sg, sg, q_data.dma_nent, i) {
			dma_addr = sg_dma_address(sg);
			dma_leng = sg_dma_len(sg);
			ret_page = sg_page(sg);
			virt_addr = kmap(ret_page) + sg->offset;
...
#else
			//assume that input test pattern is 0x01234567 or 0x89abcdef
			if(rtkemmc_swap_endian(*(u32*)(virt_addr+8))==0x01234567 || rtkemmc_swap_endian(*(u32*)(virt_addr+8))==0x89abcdef) {
				for(idx=0; idx<dma_leng; idx+=4) {
					...
					if(rtkemmc_swap_endian(*(u32*)(virt_addr+(idx2)))==0x01234567) flag1=true;
					if(rtkemmc_swap_endian(*(u32*)(virt_addr+(idx2)))==0x89abcdef) flag2=true;

					if((rtkemmc_swap_endian(*(u32*)(virt_addr+(idx2)))!=0x01234567 && rtkemmc_swap_endian(*(u32*)(virt_addr+(idx2)))!=0x89abcdef)
						|| (flag1==true && flag2==true)) {
						printk(KERN_ERR "%s: DATA error detected !!!!!!!\n", __func__);
						printk(KERN_ERR "%s: idx=%d, blk_addr=0x%x, blk_cnt=0x%x, desc_virt_addr=0x%x, dma_nent=%d\n",
								__func__, cmd_info->cmd->opcode, cmd_info->cmd->arg, cmd_info->block_count, q_data.desc, q_data.dma_nent);

						int start_idx=0;
						for(start_idx=-64; start_idx<64; start_idx+=4) {
							if(idx2+start_idx>=sg->offset && idx2+start_idx<4096) {
								printk(KERN_ERR "%s phys_addr 0x%x virt_addr 0x%x: 0x%x\n",
                                                                __func__, dma_addr+idx+start_idx, virt_addr+idx2+start_idx , rtkemmc_swap_endian(*(u32*)(virt_addr+idx2+start_idx)));
							}
						}
						kunmap(ret_page);
						asm volatile("loop1:");
						asm volatile("b loop1");
					}
				}
			}
#endif
...
#ifdef RTKMMEC_DMA_DEBUG
	rtkemmc_dump_dma_metadata(cmd_info, des_base, dma_nents);
#endif
#ifdef RTKEMMC_PHASE_TRACE
...
#if 0
		printk(KERN_INFO "%s: nent%d, dma_addr=0x%x, dma_len=%d\n", __func__, i, dma_addr, dma_leng);
#endif
		while(remain_blk_cnt) {
			...
			sync(emmc_port);
#if 0
			printk(KERN_INFO "%s - blk cnt : 0x%08x, dma_addr=0x%08x, tmp_val=0x%08x, desc[0]=0x%08x, desc[1]=0x%08x\n",
				__func__, blk_cnt2, dma_addr, tmp_val, des_base[0],des_base[1]);
#endif
			dma_addr = dma_addr+(blk_cnt2<<9);
			remain_blk_cnt -= blk_cnt2;
			des_base += 2;
...
#if defined(CONFIG_MMC_RTK_EMMC_PON)
	if(mrq->cmd->opcode==MMC_GO_IDLE_STATE)		//in Hank PON environment, rom code might occuppy the semaphore, so we release the semaphore of command 0 first
		rtkemmc_writel(0x0, emmc_port->emmc_membase+EMMC_HD_SEM);

	if(mrq->cmd->opcode==MMC_SEND_STATUS &&
		(emmc_pre_cmd==MMC_SWITCH || emmc_pre_cmd==MMC_STOP_TRANSMISSION ||
		emmc_pre_cmd==MMC_ERASE || emmc_pre_cmd==MMC_WRITE_MULTIPLE_BLOCK))	//force cmd13 cannot be separated with cmd 6, 12, 38, 25
		cmd_continuous = true;
	else if(mrq->cmd->opcode==MMC_ERASE || mrq->cmd->opcode==MMC_ERASE_GROUP_END)	//cmd 36, 38 we do not need to occupy the semaphore
		cmd_continuous = true;
	if(cmd_continuous == false && partition_pon_sem==false) {		//we need to occupy the semaphore
		while(1) {
			if(readl(emmc_port->emmc_membase+EMMC_HD_SEM)==1) break;
			else {
				printk(KERN_INFO "%s: Pon is occupying the semaphore, emmc opcode=%d, emmc argument=0x%x, \
					pon opcode=%d, pon argument=0x%x, 0x98012030=0x%x, 0x98012032=0x%x, 0x9801203c=0x%x, 0x98012814=0x%x, 0x98012818=0x%x\n",
					__func__, mrq->cmd->opcode, mrq->cmd->arg,
					CMD_IDX_MASK(readl(emmc_port->emmc_membase+EMMC_CMD_R)), readl(emmc_port->emmc_membase+EMMC_ARGUMENT_R),
					readw(emmc_port->emmc_membase+0x30), readw(emmc_port->emmc_membase+0x32),
					readw(emmc_port->emmc_membase+0x3c), readl(emmc_port->emmc_membase+0x814), readl(emmc_port->emmc_membase+0x818));
				usleep_range(10, 30);
			}
...
#if defined(CONFIG_MMC_RTK_EMMC_PON)
	if(mrq->cmd->opcode!=MMC_ERASE_GROUP_START && mrq->cmd->opcode!=MMC_ERASE_GROUP_END &&  mrq->cmd->opcode!=MMC_ERASE
		&& mrq->cmd->opcode!=MMC_SWITCH && mrq->cmd->opcode!=MMC_STOP_TRANSMISSION
		&& mrq->cmd->opcode!=MMC_WRITE_MULTIPLE_BLOCK && partition_pon_sem==false)  //cmd 35, 36, 38, 6, 12, 25 does not release the semaphore
		rtkemmc_writel(0x0, emmc_port->emmc_membase+EMMC_HD_SEM);
	else if(mrq->cmd->opcode==MMC_SWITCH && mrq->cmd->arg==0x03220301 && partition_pon_sem==false) {
		printk(KERN_ERR "S5 under PON force cmd 6 release semaphore\n");
		rtkemmc_writel(0x0, emmc_port->emmc_membase+EMMC_HD_SEM);
        }

	emmc_pre_cmd = mrq->cmd->opcode;
#endif
...
static int rtkemmc_probe(struct platform_device *pdev)
{
...
#if defined(CONFIG_MMC_RTK_EMMC_PON)
	emmc_port->norst_membase = of_iomap(emmc_node, 6);
	...
	if (gpio_is_valid(emmc_port->emmc_pon_gpio)) {
		...
		if (ret < 0)
			printk(KERN_ERR "%s: can't request emmc_pon_gpio %d\n", __func__, emmc_port->emmc_pon_gpio);
	} else {
		printk(KERN_ERR "%s: gpio %d is not valid\n", __func__, emmc_port->emmc_pon_gpio);
	}

	emmc_port->emmc_pon_toggle_gpio = of_get_gpio_flags(emmc_node, 1, NULL);
	if (gpio_is_valid(emmc_port->emmc_pon_toggle_gpio)) {
		ret = gpio_request(emmc_port->emmc_pon_toggle_gpio, "emmc_pon_toggle_gpio");
		if (ret < 0)
			printk(KERN_ERR "%s: can't request emmc_pon_toggle_gpio %d\n", __func__, emmc_port->emmc_pon_toggle_gpio);
	} else {
		printk(KERN_ERR "%s: gpio %d is not valid\n", __func__, emmc_port->emmc_pon_toggle_gpio);
	}
#endif
	...
	if (prop) {
		...
		if (prop) {
			...
			emmc_port->emmc_tuning_addr = 0x9a0e00; //use default value in the factory partition end block addr - 1025 blocks
