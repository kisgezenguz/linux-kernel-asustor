...
static int dc_do_static_post_config(VENUSFB_MACH_INFO * video_info);
...
int DC_Set_RPCAddr_Virt(struct fb_info *fb, VENUSFB_MACH_INFO *video_info, DCRT_PARAM_RPC_VIRT_ADDR *param)
{
	DC_INFO * pdc_info = (DC_INFO*)video_info->dc_info;
	DC_UNREFERENCED_PARAMETER (fb);
...
static int dc_do_static_post_config(VENUSFB_MACH_INFO * video_info)
{
	...
	if (osd_status==1){
		...
		pr_err("[%d] osd_status is disabled!!\n", __LINE__);
		return -EPERM;
	}
