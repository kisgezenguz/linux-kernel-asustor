...
static int proc_get_driver_variable(struct seq_file *m, void *v)
{
        ...
        seq_printf(m, "num_rx_queues\t0x%x\n", tp->num_rx_queues);
        seq_printf(m, "num_tx_queues\t0x%x\n", tp->num_tx_queues);
...
static inline void
rtl8125_enable_hw_queue_interrupt(struct rtl8125_private *tp, u8 message_id)
{
...
rtl8125_init_software_variable(struct net_device *dev)
{
        ...
        tp->num_tx_queues = tp->HwSuppNumTxQueues;
#else
        tp->num_tx_queues = 1;
#endif
        ...
        if (tp->HwSuppRssVer > 0) {
                ...
                tp->num_rx_queues = (tp->HwSuppNumRxQueues > rss_queue_num)?
                                    rss_queue_num : tp->HwSuppNumRxQueues;
                if (tp->num_rx_queues >= 2)
                        tp->EnableRss = 1;
        }
        ...
        if (tp->EnableRss)
                rtl8125_init_rss(tp);
#endif
        if (!tp->EnableRss || tp->num_rx_queues < 1)
                tp->num_rx_queues = 1;
...
static int rtl8125_poll_msix_ring(napi_ptr napi, napi_budget budget)
{
        ...
        if (work_done < work_to_do) {
                ...
                rtl8125_enable_hw_queue_interrupt(tp, message_id);
        }
...
static int rtl8125_open(struct net_device *dev)
{
        ...
        retval = netif_set_real_num_tx_queues(tp->dev, tp->num_tx_queues);
        if (retval < 0)
                goto err_free_all_allocated_mem;

        retval = netif_set_real_num_rx_queues(tp->dev, tp->num_rx_queues);
        if (retval < 0)
                goto err_free_all_allocated_mem;
...
rtl8125_set_rx_q_num(struct rtl8125_private *tp)
{
        ...
        rx_q_num = (u16)ilog2(tp->num_rx_queues);
...
rtl8125_hw_config(struct net_device *dev)
{
        ...
        if (tp->mcfg == CFG_METHOD_2 ||
            ...
            tp->mcfg == CFG_METHOD_5) {
                ...
                mac_ocp_data |= ((ilog2(tp->num_tx_queues) & 0x03) << 10);
...
#endif
{
        ...
        do {
                ...
                if (R8125_MULTI_RX_Q(tp)) {
                        ...
                        for (i=1; i<tp->num_rx_queues; i++)
                                other_q_status |= RTL_R16(tp, tp->isr_reg[i]);
                ...
                if (other_q_status) {
                        ...
                        for (i=1; i<tp->num_rx_queues; i++)
                                RTL_W16(tp, tp->isr_reg[i], other_q_status);
                }
                ...
                if (status & tp->intr_mask || tp->keep_intr_cnt-- > 0) {
                        u32 budget = ~(u32)0;
                        u32 const max_rx_queue = other_q_status > 0 ? tp->num_rx_queues : 1;
                        int i;
...
static void rtl8125_disable_hw_interrupt_v2(struct rtl8125_private *tp,
                u32 message_id)
{
        RTL_W32(tp, IMR_V2_CLEAR_REG_8125, BIT(message_id));
}

static void rtl8125_clear_hw_isr_v2(struct rtl8125_private *tp,
                                    u32 message_id)
{
