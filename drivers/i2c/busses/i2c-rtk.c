	...
	struct i2c_msg msgs[], int num)
{
	...
	if (handler->id == 0) {
		pctrl_state = pinctrl_lookup_state(i2c_dev->pctrl, "default");
		pinctrl_select_state(i2c_dev->pctrl, pctrl_state);
	}
#endif
...
#if defined(CONFIG_ARCH_RTD16xx) || defined(CONFIG_ARCH_RTD13xx)
			if (handler->id == 0) {
				pctrl_state = pinctrl_lookup_state(i2c_dev->pctrl, "high_speed");
				pinctrl_select_state(i2c_dev->pctrl, pctrl_state);
...
static int  rtk_i2c_init(struct rtk_i2c_dev *i2c_dev)
{
	...
	i2c_dev->pctrl = devm_pinctrl_get(i2c_dev->dev);
...
static int rtk_i2c_suspend(struct device *dev)
{
	...
	devm_pinctrl_put(i2c_dev->pctrl);
