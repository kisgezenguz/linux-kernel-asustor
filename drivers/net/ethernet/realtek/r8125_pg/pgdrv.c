	...
	if (mmio) {
		...
		case 0x8169:
			...
			if (pdev->bus->number == 0) {
				...
				rtk_pcie1_change_bar(pci_resource_start(mydev->pdev, 1));
			} else {
				mydev->base_phyaddr = 0x9803C000;
				rtk_pcie2_change_bar(pci_resource_start(mydev->pdev, 1));
			}
#else
		...
		default:
			...
			if (pdev->bus->number == 0) {
				mydev->base_phyaddr = 0x9804f000;
				rtk_pcie1_change_bar(pci_resource_start(mydev->pdev, 2));
			} else {
				mydev->base_phyaddr = 0x9803C000;
				rtk_pcie2_change_bar(pci_resource_start(mydev->pdev, 2));
			}
#else
