...
#if defined(CONFIG_ARCH_RTD13xx) && defined(CONFIG_MMC_RTK_EMMC) && defined(CONFIG_MMC_RTK_EMMC_PON)
...
#if defined(CONFIG_ARCH_RTD13xx) && defined(CONFIG_MMC_RTK_EMMC) && defined(CONFIG_MMC_RTK_EMMC_PON)
		if (md->part_type)	//PON cannot access rpmb and boot partition area
			rtkemmc_occupy_semaphore();
#endif
...
#if defined(CONFIG_ARCH_RTD13xx) && defined(CONFIG_MMC_RTK_EMMC) && defined(CONFIG_MMC_RTK_EMMC_PON)
		if (md->part_type==0)	//after swithing back to user data area, release the pon semaphore
			rtkemmc_release_semaphore();
#endif
